...
void keyboard__port_f__run(void) {
    ...
    uint8_t state = __MOV(keyboard__pins__port_f__read());
    ...
    if (changes) {
        ...
        tracer__keyboard__changes(5, changes);
        keyboard__port_f__mask &= ~changes;
        keyboard__port_f__previous_state = (keyboard__port_f__previous_state & ~changes) | (state & changes);
        keyboard__port_f__buttons__process(keyboard__port_f__previous_state, changes);
#if defined(KEYBOARD__ENCODERS) && KEYBOARD__ENCODERS == 1
