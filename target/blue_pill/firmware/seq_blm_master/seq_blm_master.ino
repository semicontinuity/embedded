...
#include "multi_blm_p1_leds_buffer__blm_master_led_updates.h"
#include "multi_blm_p1_leds_buffer__scanner.h"
...
void multi_blm_p1_leds_buffer__scanner__update_one(uint8_t matrix, uint8_t led, uint8_t r, uint8_t g, uint8_t b) {
    if (DEBUG_COMM_LEDS) {
...
void blm_master__leds__update_one(uint8_t row, uint8_t column, uint8_t color_code) {
    multi_blm_p1_leds_buffer__blm_master__update_one(row, column, color_code);
}

void blm_master__leds__update_row(uint8_t row, uint8_t is_second_half, uint8_t pattern, uint8_t color) {
    multi_blm_p1_leds_buffer__blm_master__update_row(row, is_second_half, pattern, color);
}

void blm_master__leds__update_column(uint8_t column, uint8_t is_second_half, uint8_t pattern, uint8_t color) {
    multi_blm_p1_leds_buffer__blm_master__update_column(column, is_second_half, pattern, color);
}

void blm_master__leds__update_extra_row(uint8_t is_second_half, uint8_t pattern, uint8_t color) {
    multi_blm_p1_leds_buffer__blm_master__update_extra_row(is_second_half, pattern, color);
}

void blm_master__leds__update_extra_column(uint8_t is_second_half, uint8_t pattern, uint8_t color) {
    multi_blm_p1_leds_buffer__blm_master__update_extra_column(is_second_half, pattern, color);
}

void blm_master__leds__update_extra(uint8_t is_second_half, uint8_t pattern, uint8_t color) {
    multi_blm_p1_leds_buffer__blm_master__update_extra(is_second_half, pattern, color);
}
...
void setup() {
    ...
    multi_blm_p1_leds_buffer__init();
}
...
void loop() {
    ...
    if (multi_blm_p1_leds_buffer__scanner__is_runnable()) {
        multi_blm_p1_leds_buffer__scanner__run();
    } else {
