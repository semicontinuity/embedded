...
uint8_t blm_master__sysex_handler__command_payload[768] = {};
...
void midi_parser__on_sysex_data(uint8_t data) {
    ...
    } else if (blm_master__sysex_handler__command_length == sizeof(blm_master__sysex_handler__command_header)) {
        blm_master__sysex_handler__command_header_matched = 1;
        blm_master__sysex_handler__command = data;
    } else {
        int32_t offset = blm_master__sysex_handler__command_length - sizeof(blm_master__sysex_handler__command_header) - 1;
        blm_master__sysex_handler__command_payload[offset] = data;
    }
...
void midi_parser__on_sysex_finish() {
    ...
    } else if (blm_master__sysex_handler__command_header_matched && blm_master__sysex_handler__command >= 0x10 && blm_master__sysex_handler__command <= 0x1F) {
        blm_master__sysex_handler__handle_set_palette_data(
                blm_master__sysex_handler__command - 0x10,
                blm_master__sysex_handler__command_payload,
                blm_master__sysex_handler__command_length - sizeof(blm_master__sysex_handler__command_header) - 1
        );
    } else {
        blm_master__sysex_handler__handle_invalid_command();
    }
