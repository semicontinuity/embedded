// Reads pins of logical ports A, B, C, D - corresponding to columns 0..3.
// Most pins of logical port are read from physical port IO_MATRIX__IN, and a few more pins can be read from the physical port IO_MATRIX__IN__ALT.
...
void io_matrix__in__init(void) {
        ...
        USE_PIN_AS_INPUT(IO_MATRIX__IN__ROW6__PORT, IO_MATRIX__IN__ROW6__PIN);
        ENABLE_PIN_PULLUP(IO_MATRIX__IN__ROW6__PORT, IO_MATRIX__IN__ROW6__PIN);
#endif
#if defined(IO_MATRIX__IN__ROW7__PORT) && defined(IO_MATRIX__IN__ROW7__PIN)
        USE_PIN_AS_INPUT(IO_MATRIX__IN__ROW7__PORT, IO_MATRIX__IN__ROW7__PIN);
        ENABLE_PIN_PULLUP(IO_MATRIX__IN__ROW7__PORT, IO_MATRIX__IN__ROW7__PIN);
#endif
    }
