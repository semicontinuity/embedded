...
class GCode(object):
    ...
    def deep_rect_depth_first(self, x_from, y_from, x_to, y_to, z_from=None, z_to=None, z_step=None):
        if z_from is None: z_from = self.Z_FROM
        if z_to   is None: z_to   = self.Z_DOWN
        if z_step is None: z_step = self.Z_STEP
        return (
            ...
            self.position(x_from, y_from),
            self.position_z(),
            self.deep_line(x_from, y_from, x_to  , y_from),
            self.deep_line(x_to  , y_from, x_to  , y_to),
            self.deep_line(x_to  , y_to  , x_from, y_to),
            self.deep_line(x_from, y_to  , x_from, y_from),
            self.mill_up(),
        )
