...
class GCode(object):
    ...
    def position(self, x, y):
        if self.mirror_x: x = -x
        if self.mirror_y: y = -y
    ...
    def clockwise_arc_to(self, x, y, i, j):
        ...
        if self.mirror_y:
            y = -y
            j = -j
        code = 'G03' if self.mirror_x ^ self.mirror_y else 'G02'
        return f'{code} X{x:.4f} Y{y:.4f} I{i:.4f} J{j:.4f} F{self.FEED_RATE_XY:.4f}'
