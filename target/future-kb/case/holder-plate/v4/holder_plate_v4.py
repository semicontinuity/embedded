...
CORNER_CUT_Y = -HEIGHT/2 + 2*OP_DY + RM

FRONT_X = -110

MID_X1 = FRONT_X - RM
MID_Y1 = -HEIGHT/2 + OP_DY*2 + RM
...
def milling_program(g):
    return g.program(
        g.with_spindle(
            openings_w3_h1(g, -2*OP_DY),
            openings_w3_h1(g, 2*OP_DY),

#            openings_w9_h2(g, -OP_OFFSET - 3*OP_DX, -3/2*OP_DY),
...
#            side_openings(g, WIDTH/2 + SIDE_MOUNT_OFS),
            ...
            #N
#            g.deep_line(MID_X1, MID_Y1, MID_X4, MID_Y4),
...
def openings_w3_h1(g, center_y):
    return g.grid('openings', xstart=-OP_DX, ystart=center_y, xstop=OP_DX + OP_DX, ystop=center_y + OP_DY, xstep=OP_DX, ystep=OP_DY,
          element=lambda x, y: g.deep_round_rect_at(x, y, width=OP_W - 2*RM, height=OP_H - 2*RM, r=OP_R-RM, center=True)
    )

def openings_w9_h2(g, center_x, center_y):
    return g.grid('openings', xstart=center_x - 4*OP_DX, ystart=center_y - 0.5*OP_DY, xstop=center_x + 4*OP_DX + OP_DX, ystop=center_y + 0.5*OP_DY + OP_DY, xstep=OP_DX, ystep=OP_DY,
          element=lambda x, y: g.deep_round_rect_at(x, y, width=OP_W - 2*RM, height=OP_H - 2*RM, r=OP_R-RM, center=True)
    )
