...
void spi__init(void) {
    // CPOL->0, CPHA->0, Clk/4 (?)
    OUT_1(SPI__CLK);
    USE_AS_OUTPUT(SPI__CLK);
    USE_AS_OUTPUT(SPI__MOSI);
}
...
void spi__write(uint8_t value) {
    ...
    do {
        OUT_0(SPI__CLK);
        if (value & 0x80) OUT_1(SPI__MOSI); else OUT_0(SPI__MOSI);
        value <<= 1;
        OUT_1(SPI__CLK);
    }
    while (--i);
...
void Lcd_update (void)
{
    ...
    if ( HiWaterMark < 0 )
        ...
        LcdSend( 0x4D, LCD_CMD );    // Сдвигаем картинку на 5 пикселей вверх (нестандартная команда китайца, оригинал её игнорирует)
