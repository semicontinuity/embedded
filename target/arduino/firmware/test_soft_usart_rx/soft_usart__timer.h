...
/*
#if defined(SOFT_USART__TIMER__RX__START__TEMP_REG) && TEMP_REG >= 16
...
#endif
*/
    SET_BIT_IN_IO_REG(TIMER2__COMPARE_A__INTERRUPT__ENABLE__HOST, TIMER2__COMPARE_A__INTERRUPT__ENABLE__BIT);
}
...
inline static void soft_usart__timer__rx__stop(void) {
    ...
    // To be used from interrupt only
    CLEAR_BIT_IN_IO_REG(TIMER2__COMPARE_A__INTERRUPT__ENABLE__HOST, TIMER2__COMPARE_A__INTERRUPT__ENABLE__BIT);
}

inline static void soft_usart__timer__tx__start(void) {
#if defined(SOFT_USART__TIMER__TX__START__TEMP_REG)
    // To be used from interrupt only
    register uint8_t temp asm(QUOTE(SOFT_USART__TIMER__TX__START__TEMP_REG));
    LD_IO_REG(temp, TIMER2_REG_VALUE);
...
inline static void soft_usart__timer__tx__stop(void) {
    ...
    // To be used from interrupt only
    CLEAR_BIT_IN_IO_REG(TIMER2__COMPARE_B__INTERRUPT__ENABLE__HOST, TIMER2__COMPARE_B__INTERRUPT__ENABLE__BIT);
}
