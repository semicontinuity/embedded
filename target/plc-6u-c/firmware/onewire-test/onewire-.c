...
void onewire__bitbang_thread__run(void) {
    VT_BEGIN(onewire__bitbang_thread, onewire__bitbang_thread__ip);
            // -------------------------------------------------------------------------
            VT_MARK(onewire__bitbang_thread, RESET);                        // at RESET pulse end
            ...
            VT_YIELD(onewire__bitbang_thread, onewire__bitbang_thread__ip); // should just return
            // -------------------------------------------------------------------------
            do {
                ...
                VT_YIELD(onewire__bitbang_thread, onewire__bitbang_thread__ip); // wait until T=1TQ

                VT_MARK(onewire__bitbang_thread, WRITE);

                // generate 0 for 4 TQ
                onewire__bus__set(0);
                onewire__timer__timeout__set(5*ONEWIRE__TIMER__TIME_QUANTUM);
                VT_YIELD(onewire__bitbang_thread, onewire__bitbang_thread__ip); // wait until T=5TQ

                // generate 1 on the bus, if writing 1 bit, otherwise keep 0
                uint8_t data = onewire__bitbang_thread__data;
                if (data & 0x01) onewire__bus__set(1);
                onewire__bitbang_thread__data = data >> 1;
                onewire__timer__timeout__set(61*ONEWIRE__TIMER__TIME_QUANTUM);
                VT_YIELD(onewire__bitbang_thread, onewire__bitbang_thread__ip); // wait until T=61TQ
            }
...
bool onewire__thread__is_alive(void) {
    return (bool)onewire__thread__alive;
}
 ...
 * If presence pulse is generated, onewire__bitbang_thread__data will not equal to 0xFF.
 */
void onewire__thread__reset_bus(void) {
//    onewire__bitbang_thread__data__set(0xFF);
    onewire__bus__set(0);

    onewire__timer__timeout__set(255);
    onewire__bitbang_thread__start();
    onewire__timer__start(ONEWIRE__TIMER__FAST_CONF);
}
...
void onewire__thread__run(void) {
    VT_BEGIN(onewire__thread, onewire__thread__ip);
            ...
            // thread will not be scheduled until bit-bang thread finishes:
            // wait until reset pulse is generated and presence pulse is read.
