...
// the bus is connected via inverting buffer.
//
// Parameters:
//     ONEWIRE__OUT__PORT, ONEWIRE__OUT__PIN:
//         AVR port and pin to which inverting 1-wire bus driver is attached
...
void onewire__bus__init(void) {
    USE_AS_OUTPUT(ONEWIRE__OUT);	// Will output 0 => bus high
}


void onewire__bus__set(const uint8_t value) {
    if (value) {
        OUT_0(ONEWIRE__OUT);  // Buffer pull-up pulls up the bus high.
    }
    else {
        OUT_1(ONEWIRE__OUT);  // Buffer inverts and drives the bus low.
    }
}
