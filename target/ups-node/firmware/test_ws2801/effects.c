...
extern uint8_t pixel_data[WS2801_BUFFER_LEN];
...
static long random_l(long a, long b) {
  return (random() % (b - a)) + a;
}

static void point_4(uint16_t p, uint8_t r, uint8_t g, uint8_t b) {
  pixel_data[p * 3] = r;
  pixel_data[p * 3 + 1] = g;
  pixel_data[p * 3 + 2] = b;
}

static void point_2(uint16_t p, uint32_t c) {
  pixel_data[p * 3] = (uint8_t)(c >> 16);
  pixel_data[p * 3 + 1] = (uint8_t)(c >> 8);
  pixel_data[p * 3 + 2] = (uint8_t)c;
}

static void point_3(unsigned char x, unsigned char y, uint32_t c) {
  if (y & 1) {
    x = WIDTH - x; // odd numbered line - need to flip x
  }
  point_2((int)y * WIDTH + (int)x, c);
}
...
static void blank(void) {
  for (int i = 0; i < N_PIXELS; ++i) point_2(i, 0);
}
...
static void set_color(int r, int g, int b) {
  for (int i = 0; i < N_PIXELS; ++i) point_4(i, r, g, b);
}
...
static void flashes(void) {
  for (int tmp = 0; tmp < 100; ++tmp) {
    for (int c = 0; c < 0; ++c) {
      point_3(random_l(0, WIDTH), random_l(0, HEIGHT), Color(255, 255, 255));
    }
    if (random_l(0, 10) == 0) rect(0, 0, WIDTH, HEIGHT, random_color());
    flip();
    delay(20);
  }
}

static void bright_flash(void) {
...
static void rectangles(int wait) {
  ...
  for (uint8_t tmp = 0; tmp < 100; ++tmp) {
    blank();
    char x0 = random_l(0, WIDTH), x1 = random_l(0, WIDTH), y0 = random_l(0, HEIGHT), y1 = random_l(0, HEIGHT);
...
static void chase(void) {
  for (int pos = 0; pos < N_PIXELS; ++pos) {
    point_4((pos+N_PIXELS-1)%N_PIXELS, 0, 0, 0);
    point_4(pos, 255, 255, 255);
    show();
    delay(20);
  }
  for (int pos = 0; pos < N_PIXELS; ++pos) {
    point_4((pos+N_PIXELS-1)%N_PIXELS, 0, 0, 0);
    point_4(pos, randbyte(), randbyte(), randbyte());
    show();
    delay(20);
  }
}

static void trailchase(void) {
  for (int pos = 0; pos < N_PIXELS; ++pos) {
    point_4(pos, randbyte(), randbyte(), randbyte());
    show();
    delay(20);
...
void run_effects(void) {
  ...
  for (int p = 0; p < N_PIXELS; ++p) {
    point_4(p, randbyte(), randbyte(), randbyte());
  }
  show();
  return;
...
//    set_color(255, 255, 255); // FULL WHITE
    for (int x = 0; x < 30; ++x) point_2(random_l(0, N_PIXELS), Color(255, 255, 255));
    show();
    delay(wait);
  }
  for (tmp = 0; tmp < 100; ++tmp) {
    blank();
    for (int x = 0; x < 30; ++x) point_2(random_l(0, N_PIXELS), random_color());
    show();
    delay(wait);
  }
  return;
  ...
  //  }
    for (int x = 0; x < 10; ++x) point_2(random_l(0, N_PIXELS), random_color());
...
static void rainbowCycle(uint8_t wait) {
  ...
  for (j=0; j < 256 * 5; j++) {     // 5 cycles of all 25 colors in the wheel
    for (i=0; i < N_PIXELS; i++) {
      ...
      point_2(i, Wheel( ((i * 256 / N_PIXELS) + j) % 256) );
    }
    show();   // write all the pixels out
    delay(wait);
  }
