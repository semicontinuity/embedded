...
def milling_program(gcode):
    return gcode.program(
        gcode.with_spindle(
            gcode.grid('button openings', xstart=12.5,  ystart=12.5, ystop=100+12.5, ystep=25,
              element=lambda x, y: gcode.deep_round_rect_at(x, y, width=O_SIZE-2*RM, height=O_SIZE-2*RM, r=O_R-RM, center=True)
            ),

            gcode.grid('button openings', xstart=50+12.5, xstop=50+200 + 12.5, xstep=25,  ystart=12.5, ystop=112.5, ystep=25,
              element=lambda x, y: gcode.deep_round_rect_at(x, y, width=O_SIZE-2*RM, height=O_SIZE-2*RM, r=O_R-RM, center=True)
            ),
            ...
            (
                ...
                for x in [12.5, 50+12.5, 50+87.5, 50+112.5, 50+187.5] for y in [25, 75]
            ),
        )
    )
