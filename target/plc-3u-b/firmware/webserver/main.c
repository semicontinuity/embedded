...
unsigned int var_array[MAX_VAR_ARRAY] = {10,50,30,0};
...
//    printf_P(PSTR("Compiliert mit GCC Version "__VERSION__"\r\n"));
	...
	f16_init();
/*
        uint8_t r = sd_raw_read(256, buffer, sizeof(buffer));
        printf_P(PSTR("Read result %d\n\r"), r);
        printf_P(PSTR("[-2] %d\n\r"), buffer[254]);
        printf_P(PSTR("[-1] %d\n\r"), buffer[255]);
*/
        f16_check();
...
*/
		...
		*/
		if (sensors_pins_changed()) {

			// Eingang hat sich geändert

			if (sensor1_pin_changed()) {
				usart_write("\r\nSensor 1 signal.");
			}

			if (sensor2_pin_changed()) {
				usart_write("\r\nSensor 2 signal.");
			}

			if (sensor3_pin_changed()) {
				usart_write("\r\nSensor 3 signal.");
			}

			if (sensor4_pin_changed()) {
				usart_write("\r\nSensor 4 signal.");
			}
