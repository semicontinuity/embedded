...
printf_P(PSTR("FAT16 open\n\r"));
    ...
    if(i >= FAT16_FS_COUNT) {
printf_P(PSTR("FAT16 open problem\n\r"));
        return 0;
    } 
#endif
...
printf_P(PSTR("FAT16 read header\n\r"));
    if(!fat16_read_header(fs))
    {
        ...
        fs->partition = 0;
#endif
printf_P(PSTR("FAT16 read header problem\n\r"));
        return 0;
    }
...
printf_P(PSTR("FAT16 read FAT %lX %d\n\r"), partition_offset + 0x0b, sizeof(buffer));

    if(!partition->device_read(partition_offset + 0x0b, buffer, sizeof(buffer))) {
printf_P(PSTR("FAT16 device read problem\n\r"));
        return 0;
    }
