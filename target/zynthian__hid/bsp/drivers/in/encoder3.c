...
uint8_t encoder3__current_state() {
...
void encoder3__start(void) {
    __asm__ __volatile__("encoder3__start:");
    encoder3__state = encoder3__current_state();
}

void encoder3__run(void) {
    __asm__ __volatile__("encoder3__run:");
    if (!encoder3__debounce_timer__is_started()) {
        uint8_t current_state = encoder3__current_state();
