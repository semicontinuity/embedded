...
void kernel__send(const uint8_t* data, const uint8_t size, const uint8_t dst, const uint8_t obj_id, const uint8_t port) {
    mcp251x_message_id *id = &kernel__frame.header.id;
    UCAN_SET_DST(*id, dst);
    UCAN_SET_VALUE_OBJID(*id, obj_id);
    UCAN_SET_PORT(*id, port);
    kernel__send_response(size, data);
}
